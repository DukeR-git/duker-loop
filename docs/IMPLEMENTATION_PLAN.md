...
| 34 | Fleet view | Claude-Code/pi-subagents-style child list + live viewer (`activity.ts`, `fleet.ts`). The loop, `/duker run` and `/duker init` report through optional `ChildHooks` on the `ProgressSink` (`childStart` / `childEvent` / `childEnd`) into an in-memory `Activity` (current run's rows = the step's phase chain incl. pending placeholders; bounded transcript per child: tool calls, tool results, completed and streamed assistant text, retries, stderr). The runtime gained `textDelta` (message_update) and `toolEnd` (tool_execution_end) events for it. UI mechanics copied from tintinweb/pi-subagents' FleetView: render-only `belowEditor` widget; keys via `ctx.ui.onTerminalInput` (fires before the editor, may consume), gated on an empty prompt and the editor holding focus (`focusedComponent instanceof Editor`); ↓/← activate, ↑/↓ select, Enter opens a `ctx.ui.custom({ overlay: true })` viewer (scroll, follow-tail, `x x` aborts the run), Esc returns. `/duker watch` and `ctrl+shift+d` open the viewer directly. Without a UI (or a pi lacking `onTerminalInput`) nothing is registered. |
...
│       ├── activity.ts     Activity: in-memory model of the current run's children (rows + transcripts), ChildHooks
│       ├── fleet.ts        DukerFleet (below-editor list, arrow-key navigation) + ChildViewer (live overlay)
...
| 10 fleet view | done | `activity.ts` + `fleet.ts` + `ChildHooks` on the sink (decision #34). Tests: activity model (pending chain, transcript, caps, run-id rollover), fleet keys through a fake `ctx.ui` (activation gates, navigation, overlay open/close, abort arming, scroll/follow, dispose, ui without `onTerminalInput`), one real fake-pi loop driving the list through all seven phases, and the command surface (`/duker watch`, the shortcut, widget lifecycle through `ctx.ui`). Not yet seen in a real terminal — the `onTerminalInput` / `setWidget` / `custom` contracts are taken from pi-subagents' source; verify in the container. |
